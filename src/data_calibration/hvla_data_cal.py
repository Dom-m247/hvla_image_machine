...
def data_calibration(options:Options):
  ...
  if not Path(options.calibrated_filename+'.ms').is_dir():
    if not options.target_is_phase_cal:
      main_calibrations.primary_calibration(options)
    else: 
...
def project_code_from_archives(archive_files):
  ...
  radio_search takes it from the selected observation; a segment picked off disk
  has no such record, so use the folder the files sit in (both DelosDownload and
  the data_archive layout group a segment under <proj_code>/), falling back to
  the leading characters of the file name as the single-archive path does.
  """
  first = Path(archive_files[0])
  parent = first.parent.name
  if parent and parent != DATA_ARCHIVE.strip('/'):
    return parent
  return first.name[:5]
...
def convert_to_ms(archive,options):
  ...
  `archive` may be a single archive/MS path or a list of raw archive files --
  a segment downloaded by radio_search, or one picked off disk in the GUI/CLI --
  which are all imported into one MS.
  """
  #a multi-file segment: raw VLA archive files (downloaded by radio_search, or
  #picked locally) that importvla concatenates into one MS
...
def pre_data_calibration(options:Options):
  ...
  options.flux_cal.initial_ms_fieldID = options.flux_cal.find_fieldID(options.init_data)
...
def build_setjy(options):
  ...
  if manual := getattr(options, 'flux_cal_manual', None):
    print(f"setjy: manual flux {manual['flux']} Jy at {manual['reffreq']} on {amp_field}")
    #fluxdensity is [I,Q,U,V]; the casatasks stub types it as a bare int
    ct.setjy(vis=visfile,field=amp_field,standard='manual',
             fluxdensity=[float(manual['flux']),0,0,0],  # type: ignore[arg-type]
             reffreq=manual['reffreq'],usescratch=True,scalebychan=True,spw='')
    return
  use_model = options.flux_cal.model
...
def generate_file_names(options:Options):
  ...
  options.initial_calibration_filename = MS_SUB_PATH + options.observation_data.obs_info.project + '_' +PRIMARY_CAL_MS
...
def print_calibration_summary(options:Options):
  ...
  The phase calibrator is only shown when the target is not acting as its own.'''
  width = 60
  ...
  _source_block("Flux Calibrator", options.flux_cal)
  if options.target_is_phase_cal:
    print('-' * width)
    print(" Phase Calibrator")
    print("   (none -- the target is bright enough to serve as its own)")
  else:
...
def set_calibrators(options:Options):
  ...
  #Each source_info applies its own decision mode (see classes/decisions.py).
  options.source_ids = source_info(options,type="target",name=options.source)
  options.flux_cal = source_info(options,type="flux_calibrator")
  options.source_ids.check_target_as_phase_cal(options)
  if not options.target_is_phase_cal:
